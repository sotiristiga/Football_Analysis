...
dataset2223 = pd.read_csv(
    f"https://raw.githubusercontent.com/sotiristiga/Football_Analysis/refs/heads/main/superleague2223.csv")
dataset2324 = pd.read_csv(
    f"https://raw.githubusercontent.com/sotiristiga/Football_Analysis/refs/heads/main/superleague2324.csv")
dataset2425 = pd.read_csv(
    f"https://raw.githubusercontent.com/sotiristiga/Football_Analysis/refs/heads/main/superleague2425.csv")
...
f1,f2=st.columns(2)
with f1:
    st.write("## First Player filters")
    col1,col2,=st.columns(2)
    with col1:
        selected_Player1 = st.selectbox("Choose First Player:",
                                                dataset['Player'].reset_index().sort_values('Player')['Player'].unique())
        selected_season1 = st.selectbox("Season (First Player):", ['All', '2022-2023', "2023-2024", '2024-2025'],
                                                index=0)
        selected_phase1 = st.selectbox("Phase (First Player):", ['Regular Season', 'Play offs', "Play In",'Play out', 'All'],
                                               index=4)
    with col2:
        selected_round1 = st.selectbox("Round (First Player):", ['First Round', 'Second Round', 'All'], index=2)
        selected_ha1 = st.selectbox("Home or Away games (First Player):", ['Away', 'Home', 'All'], index=2)


        selected_wl1 = st.selectbox("Result (First Player):", ['Win', 'Draw', 'Lose', 'All'], index=3)


with f2:
    st.write("## Second Player filters")
    col3, col4, = st.columns(2)
    with col3:
        selected_Player2 = st.selectbox("Choose Second Player:",
                                                dataset['Player'].reset_index().sort_values('Player')['Player'].unique(),
                                                index=1)
        selected_season2 = st.selectbox("Season (Second Player):", ['All', '2022-2023', "2023-2024", '2024-2025'],
                                                index=0)
        selected_phase2 = st.selectbox("Phase (Second Player):", ['Regular Season', 'Play offs',"Play In", 'Play out', 'All'],
                                               index=4)
    with col4:
        selected_round2 = st.selectbox("Round (Second Player):", ['First Round', 'Second Round', 'All'], index=2)
        selected_ha2 = st.selectbox("Home or Away games (Second Player):", ['Away', 'Home', 'All'], index=2)


        selected_wl2 = st.selectbox("Result (Second Player):", ['Win', 'Draw', 'Lose', 'All'], index=3)
...
try:
    ...
    def computeplayerstats(data, player, sls, slp, slr, slha, slwl):
        ...
        computeplayerstats_total["Dribble(%)"] = (
                100 * computeplayerstats_total["Dribble attempts succ"] / computeplayerstats_total[
        ...
        computeplayerstats_total["Long balls(%)"] = (
                100 * computeplayerstats_total["Accurate Long balls"] / computeplayerstats_total[
            "Total Long balls"]).round(1)
        computeplayerstats_total["Long balls(%)"] = computeplayerstats_total["Long balls(%)"].fillna(0)
        computeplayerstats_total["Duels(%)"] = (
                    100 * computeplayerstats_total["Duels won"] / computeplayerstats_total["Duels"]).round(1)
        ...
        computeplayerstats_mean["Dribble(%)"] = (
                100 * computeplayerstats_mean["Dribble attempts succ"] / computeplayerstats_mean[
            "Dribble attempts"]).round(1)
        ...
        computeplayerstats_mean["Long balls(%)"] = (
                100 * computeplayerstats_mean["Accurate Long balls"] / computeplayerstats_mean[
            "Total Long balls"]).round(1)
        computeplayerstats_mean["Long balls(%)"] = computeplayerstats_mean["Long balls(%)"].fillna(0)
        computeplayerstats_mean["Duels(%)"] = (
                    100 * computeplayerstats_mean["Duels won"] / computeplayerstats_mean["Duels"]).round(1)
        ...
        colsplus = ['Assists', 'Shots on target', 'Shots off target', 'Shots blocked',
                    ...
                    'Was fouled']
        colsminus = ['Red card', 'Big chances missed', 'Penalty miss', 'Hit woodwork', 'Dribbled past',
                     'Penalty committed',
        ...
        computeplayerstats_mean_gk = computeplayerstats_mean[
            ['Player', 'Saves', 'Punches', 'Runs out succ', 'Runs out', "Runs out(%)", 'High claims',
        ...
        computeplayerstats_mean_sel['Player'] = computeplayerstats_mean_sel[
    ...
    finaltotal = pd.concat([computeplayerstats(dataset_filter1, selected_Player1, select_season1, select_phase1,
                                               select_round1, select_ha1, select_wl1)[0],
                            computeplayerstats(dataset_filter2, selected_Player2, select_season2, select_phase2,
                                               select_round2, select_ha2, select_wl2)[0]])

    finalmean = pd.concat([computeplayerstats(dataset_filter1, selected_Player1, select_season1, select_phase1,
                                              select_round1, select_ha1, select_wl1)[1],
                           computeplayerstats(dataset_filter2, selected_Player2, select_season2, select_phase2,
                                              select_round2, select_ha2, select_wl2)[1]])
    player1_ratings = \
    computeplayerstats(dataset_filter1, selected_Player1, select_season1, select_phase1, select_round1, select_ha1,
                       select_wl1)[2]
    player2_ratings = \
    computeplayerstats(dataset_filter2, selected_Player2, select_season2, select_phase2, select_round2, select_ha2,
                       select_wl2)[2]
    ...
    with player1:
        ...
        st.write(
            '##### Position: ' + dataset_filter1.loc[dataset_filter1.Player == selected_Player1]['Position'].unique()[
                0])
    ...
    with player2:
        ...
        st.write(
            '##### Position: ' + dataset_filter2.loc[dataset_filter2.Player == selected_Player2]['Position'].unique()[
                0])
    ...
    with totalbasic:
        basic_stats_total = finaltotal[
            ['Player', 'Games', 'Minutes played', 'Goals', 'Assists', 'Yellow card', 'Red card']]
    ...
    attack1 = player1_ratings[
        ['Rating Shots on target', 'Rating Shots off target', 'Rating Shots blocked', 'Rating Dribble attempts succ',
         'Rating Dribble attempts', "Rating Dribble(%)", 'Rating Penalty won', 'Rating Big chances missed',
         'Rating Penalty miss',
         'Rating Hit woodwork', 'Rating Offsides', 'Rating Goals']]
    attack2 = player2_ratings[
        ['Rating Shots on target', 'Rating Shots off target', 'Rating Shots blocked', 'Rating Dribble attempts succ',
         'Rating Dribble attempts', "Rating Dribble(%)", 'Rating Penalty won', 'Rating Big chances missed',
         'Rating Penalty miss',
         'Rating Hit woodwork', 'Rating Offsides', 'Rating Goals']]
    attack1sel = (attack1[['Rating Shots on target', 'Rating Shots off target', 'Rating Shots blocked',
                           'Rating Dribble attempts succ', "Rating Dribble(%)", 'Rating Big chances missed']]
                  .rename(
        columns={'Rating Shots on target': 'Shots<br>on<br>target', 'Rating Shots off target': 'Shots<br>off<br>target',
                 'Rating Shots blocked': 'Shots<br>blocked',
                 'Rating Dribble attempts succ': 'Dribble<br>attempts<br>succ', "Rating Dribble(%)": "Dribble(%)",
                 'Rating Big chances missed': 'Big<br>chances<br>missed'
                 })
                  .melt())
    attack1sel['variable'] = attack1sel['variable'].replace('Rating ', '')
    attack2sel = (attack2[['Rating Shots on target', 'Rating Shots off target', 'Rating Shots blocked',
                           'Rating Dribble attempts succ', "Rating Dribble(%)", 'Rating Big chances missed']]
                  .rename(
        columns={'Rating Shots on target': 'Shots<br>on<br>target', 'Rating Shots off target': 'Shots<br>off<br>target',
                 'Rating Shots blocked': 'Shots<br>blocked',
    ...
    with starattack:
        ...
        staratt.add_trace(go.Scatterpolar(r=attack1sel['value'], theta=attack1sel['variable'],
                                          fill='toself',
                                          name=selected_Player1 + " " + select_season1 + " " + select_phase1 + " " + select_round1 + " " + select_ha1 + " " + select_wl1,
                                          line_color='peru',
    ...
    defence1 = player1_ratings[
        ['Rating Defensive actions', 'Rating Clearances', 'Rating Blocked shots', 'Rating Interceptions',
         'Rating Total tackles',
         'Rating Dribbled past',
         'Rating Penalty committed', 'Rating Own goals', 'Rating Last man tackle', 'Rating Error led to shot',
         'Rating Clearance off line',
         'Rating Error led to goal']]
    defence2 = player2_ratings[
        ['Rating Defensive actions', 'Rating Clearances', 'Rating Blocked shots', 'Rating Interceptions',
         'Rating Total tackles',
         'Rating Dribbled past',
         'Rating Penalty committed', 'Rating Own goals', 'Rating Last man tackle', 'Rating Error led to shot',
         ...
         'Rating Error led to goal']]
    defence1sel = (
        defence1[['Rating Clearances', 'Rating Blocked shots', 'Rating Interceptions', 'Rating Total tackles',
                  'Rating Dribbled past']]
        .rename(columns={'Rating Clearances': 'Clearances', 'Rating Blocked shots': 'Blocked<br>shots',
    ...
    defence2sel = (
        defence2[['Rating Clearances', 'Rating Blocked shots', 'Rating Interceptions', 'Rating Total tackles',
                  'Rating Dribbled past']]
        .rename(columns={'Rating Clearances': 'Clearances', 'Rating Blocked shots': 'Blocked<br>shots',
                         'Rating Interceptions': 'Interceptions',
                         'Rating Total tackles': 'Total<br>tackles', "Rating Dribbled past": "Dribbled<br>past"})
    ...
    with totalduels:
        duels_stats_total = finaltotal[
            ['Player', 'Duels', 'Duels won', "Duels(%)", 'Ground duels', 'Ground duels won', 'Ground duels(%)',
             'Aerial duels', 'Aerial duels won', 'Aerial duels(%)',
    ...
    duels1 = player1_ratings[
        ['Rating Duels', 'Rating Duels won', "Rating Duels(%)", 'Rating Ground duels', 'Rating Ground duels won',
         'Rating Ground duels(%)', 'Rating Aerial duels',
         'Rating Aerial duels won', 'Rating Aerial duels(%)', 'Rating Possession lost', 'Rating Fouls',
         'Rating Was fouled']]
    duels2 = player2_ratings[
        ['Rating Duels', 'Rating Duels won', "Rating Duels(%)", 'Rating Ground duels', 'Rating Ground duels won',
         'Rating Ground duels(%)', 'Rating Aerial duels',
         'Rating Aerial duels won', 'Rating Aerial duels(%)', 'Rating Possession lost', 'Rating Fouls',
         'Rating Was fouled']]
    duels1sel = (duels1[['Rating Duels won', "Rating Duels(%)", 'Rating Ground duels won', 'Rating Ground duels(%)',
                         'Rating Aerial duels won', 'Rating Aerial duels(%)',
                         'Rating Possession lost']]
                 .rename(columns={'Rating Duels won': 'Duels<br>won', 'Rating Duels(%)': 'Duels(%)',
                                  'Rating Ground duels won': 'Ground<br>duels<br>won',
                                  'Rating Ground duels(%)': 'Ground<br>duels(%)',
                                  "Rating Aerial duels won": "Aerial<br>duels<br>won",
                                  'Rating Aerial duels(%)': 'Aerial<br>duels(%)',
                                  'Rating Possession lost': 'Possession<br>lost'})
                 .melt())
    duels1sel['variable'] = duels1sel['variable'].replace('Rating ', '')
    duels2sel = (duels2[['Rating Duels won', "Rating Duels(%)", 'Rating Ground duels won', 'Rating Ground duels(%)',
                         'Rating Aerial duels won', 'Rating Aerial duels(%)',
                         'Rating Possession lost']]
                 .rename(columns={'Rating Duels won': 'Duels<br>won', 'Rating Duels(%)': 'Duels(%)',
                                  'Rating Ground duels won': 'Ground<br>duels<br>won',
    ...
    pass1 = player1_ratings[
        ['Rating Touches', 'Rating Accurate passes', 'Rating Total passes', "Rating Passes(%)", 'Rating Key passes',
         'Rating Accurate Crosses',
         'Rating Total Crosses', "Rating Crosses(%)", 'Rating Accurate Long balls', 'Rating Total Long balls',
         "Rating Long balls(%)",
         'Rating Big chances created', 'Rating Assists']]
    pass2 = player2_ratings[
        ['Rating Touches', 'Rating Accurate passes', 'Rating Total passes', "Rating Passes(%)", 'Rating Key passes',
         'Rating Accurate Crosses',
         'Rating Total Crosses', "Rating Crosses(%)", 'Rating Accurate Long balls', 'Rating Total Long balls',
         "Rating Long balls(%)",
         'Rating Big chances created', 'Rating Assists']]
    pass1sel = (pass1[['Rating Touches', 'Rating Accurate passes', "Rating Passes(%)", 'Rating Key passes',
                       'Rating Accurate Crosses', "Rating Crosses(%)", 'Rating Accurate Long balls',
                       "Rating Long balls(%)", 'Rating Big chances created']]
                .rename(columns={'Rating Touches': 'Touches', 'Rating Accurate passes': 'Accurate<br>passes',
                                 'Rating Passes(%)': 'Passes(%)',
                                 'Rating Key passes': 'Key<br>passes', 'Rating Accurate Crosses': "Accurate<br>Crosses",
                                 'Rating Crosses(%)': 'Crosses(%)',
                                 'Rating Accurate Long balls': 'Accurate<br>Long<br>balls',
                                 "Rating Long balls(%)": "Long<br>balls(%)",
                                 'Rating Big chances created': 'Big<br>chances<br>created'})
                .melt())
    pass1sel['variable'] = pass1sel['variable'].replace('Rating ', '')
    pass2sel = (pass2[['Rating Touches', 'Rating Accurate passes', "Rating Passes(%)", 'Rating Key passes',
                       'Rating Accurate Crosses', "Rating Crosses(%)", 'Rating Accurate Long balls',
                       "Rating Long balls(%)", 'Rating Big chances created']]
                .rename(columns={'Rating Touches': 'Touches', 'Rating Accurate passes': 'Accurate<br>passes',
                                 'Rating Passes(%)': 'Passes(%)',
    ...
    gk1 = player1_ratings[
        ['Rating Saves', 'Rating Punches', 'Rating Runs out succ', 'Rating Runs out', 'Rating Runs out(%)',
         'Rating High claims', 'Rating Saves from inside box',
         'Rating Penalties saved']].replace(np.nan,0)
    gk2 = player2_ratings[
        ['Rating Saves', 'Rating Punches', 'Rating Runs out succ', 'Rating Runs out', 'Rating Runs out(%)',
         'Rating High claims', 'Rating Saves from inside box',
         'Rating Penalties saved']].replace(np.nan,0)
    gk1sel = (gk1[['Rating Saves', 'Rating Punches', 'Rating Runs out succ', 'Rating Runs out(%)', 'Rating High claims',
                   'Rating Saves from inside box']]
              .rename(
        columns={'Rating Saves': 'Saves', 'Rating Punches': 'Punches', 'Rating Runs out succ': 'Runs<br>out<br>succ',
                 'Rating Runs out(%)': 'Runs<br>out(%)', 'Rating High claims': "High<br>claims",
                 'Rating Saves from inside box': 'Saves<br>from<br>inside<br>box'})
              .melt())
    gk1sel['variable'] = gk1sel['variable'].replace('Rating ', '')
    gk2sel = (gk2[['Rating Saves', 'Rating Punches', 'Rating Runs out succ', 'Rating Runs out(%)', 'Rating High claims',
                   'Rating Saves from inside box']]
              .rename(
        columns={'Rating Saves': 'Saves', 'Rating Punches': 'Punches', 'Rating Runs out succ': 'Runs<br>out<br>succ',
                 'Rating Runs out(%)': 'Runs<br>out(%)', 'Rating High claims': "High<br>claims",
